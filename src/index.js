...
const PATCHES = [
  {
    ...
    replace:
      "function codexSessionsPatchNormalizePath(e){if(typeof e!=\"string\"||e.length===0||e===\"~\")return null;let r=e.replace(/\\\\/g,\"/\").replace(/\\/+$/,\"\");return/^[a-zA-Z]:\\//.test(r)||r.startsWith(\"//\")?r.toLowerCase():r}function codexSessionsPatchCwdBelongsToWorkspace(e,r){let n=codexSessionsPatchNormalizePath(e);if(n==null)return!1;return r.some(e=>{let r=codexSessionsPatchNormalizePath(e);return r!=null&&(n===r||n.startsWith(`${r}/`))})}function codexSessionsPatchFilterThreadListResponse(e){try{let r=Pl.workspace.workspaceFolders?.map(e=>e.uri.fsPath)??[];if(r.length===0||e==null||e.error||e.result==null||!Array.isArray(e.result.data))return e;let n=e.result.data.filter(e=>codexSessionsPatchCwdBelongsToWorkspace(e?.cwd,r));return{...e,result:{...e.result,data:n}}}catch{return e}}var Mle=\"1\",Ole=10*6e4,NE=class{",
  },
  {
    ...
    replace:
      "pendingNotifications=[];internalNotificationHandlers=new Set;ephemeralThreadTimeouts=new Map;pendingPrewarmedThreadStartRequestIds=new Set;threadListRequestIds=new Set;prewarmedThreads=new Ch",
  },
  {
    ...
    replace:
      "sendProviderRequest(e,r,n,o,i){let s=`${e}:${r}`;i&&this.pendingPrewarmedThreadStartRequestIds.add(s),n===\"thread/list\"&&(this.threadListRequestIds.add(s),o={...o,limit:Math.max(Number(o?.limit)||0,500)});let a={id:s,method:n,params:o};if(this.recordLastOutboundMethod(n),this.sendMessage(a)&&n===\"turn/start\"){let c=Jx(o);c!=null&&this.prewarmedThreads.publishThreadStarted(c)}}",
  },
  {
...
function applyPatches(options) {
  ...
  const backupDir =
    changedFiles.length > 0 ? createBackup(scan.extensionRoot, changedFiles, options) : null;
